...
void CameraSceneNode::do_CALCULATE_TRANSFORMATIONS(Events::Event *pEvt)
{
    ...
    m_aspect = (PrimitiveTypes::Float32)(m_pContext->getGPUScreen()->getWidth()) / (PrimitiveTypes::Float32)(m_pContext->getGPUScreen()->getHeight());
    
    m_verticalFov = 0.33f * PrimitiveTypes::Constants::c_Pi_F32;
    if (m_aspect < 1.0f)
    {
        //ios portrait view
        static PrimitiveTypes::Float32 factor = 0.5f;
        m_verticalFov *= factor;
    }

	m_viewToProjectedTransform = CameraOps::CreateProjectionMatrix(m_verticalFov, 
		m_aspect,
		m_near, m_far);
	...
	forward.normalize();
	up.normalize();
    m_frustrum = CameraOps::CreateFrustrum(pos, forward, up, m_verticalFov, m_aspect, m_near, m_far);
